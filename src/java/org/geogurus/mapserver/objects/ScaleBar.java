...
public class ScaleBar extends MapServerObject  implements java.io.Serializable {
    ...
    /** Empty constructor */
    public ScaleBar() {
        this(null, null, null, null, null, null, (byte)0);
    }
    
    /** Creates a new instance of ScaleBar */
    public ScaleBar(RGB backgroundColor_, RGB color_, RGB imageColor_,
    ...
    public void setInterlace(boolean interlace_)       {interlace = interlace_;}
    public void setIntervals(int intervals_)        {intervals = intervals_;}
    ...
    public void setStyle(int style_)               {style = style_;}
    public void setTransparent(byte transparent_){transparent = transparent_;}
    public void setUnits(byte units_)               {units = units_;}
    ...
    public boolean isInterlace()          {return interlace;}
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (tokens[0].equalsIgnoreCase("BACKGROUNDCOLOR")) {
                    ...
                    if (tokens.length<2) return false;
                    tokens[1] = ConversionUtilities.removeDoubleQuotes(tokens[1]);
                    if (tokens[1].equalsIgnoreCase("ON") || tokens[1].equalsIgnoreCase("TRUE")) transparent = this.ON;
                    else if (tokens[1].equalsIgnoreCase("OFF") || tokens[1].equalsIgnoreCase("FALSE")) transparent = this.OFF;
                    else return false;
                }
                else if (tokens[0].equalsIgnoreCase("UNITS")) {
                    if (tokens.length<2) return false;
                    tokens[1] = ConversionUtilities.removeDoubleQuotes(tokens[1]);
                    if (tokens[1].equalsIgnoreCase("FEET")) units = this.FEET;
                    else if (tokens[1].equalsIgnoreCase("INCHES")) units = this.INCHES;
                    else if (tokens[1].equalsIgnoreCase("KILOMETERS")) units = this.KILOMETERS;
                    else if (tokens[1].equalsIgnoreCase("METERS")) units = this.METERS;
                    else if (tokens[1].equalsIgnoreCase("MILES")) units = this.MILES;
                    else return false;
                }
            ...
            System.out.println("ScaleBar.load(). Exception: " +  e.getMessage());
            e.printStackTrace();
            return false;
        }
        return result;
    ...
    public synchronized boolean saveAsMapFile(java.io.BufferedWriter bw) {
        ...
        try {
            ...
            if (size!=null)             bw.write("\t\t size "+size.width+" "+size.height+"\n");
            switch (status) {
            ...
            switch (units) {
                case FEET:              bw.write("\t\t units FEET\n"); break;
                case INCHES:            bw.write("\t\t units INCHES\n"); break;
                case KILOMETERS:        bw.write("\t\t units KILOMETERS\n"); break;
                case METERS:            bw.write("\t\t units METERS\n"); break;
                case MILES:             bw.write("\t\t units MILES\n"); break;
            }
