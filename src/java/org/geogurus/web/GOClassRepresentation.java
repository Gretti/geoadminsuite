...
public class GOClassRepresentation extends BaseServlet {
    ...
    public void init(){
        ...
        vp1.add(new Pattern("gasdefsympoint", 8));
    ...
    public void process(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (genColor != null && genColor.length() > 0) {cl.setColor(new RGB(genColor));} 
        else if (fgColor != null && fgColor.length() > 0) {cl.setColor(new RGB(fgColor));}
        
        if (bgColor != null && bgColor.length() > 0) {cl.setBackgroundColor(new RGB(bgColor));}
        
        if (outColor != null && outColor.length() > 0) {cl.setOutlineColor(new RGB(outColor));}
        
        if (txtColor != null && txtColor.length() > 0 && cl.getLabel() != null) {cl.getLabel().setColor(new RGB(fgColor));}
        ...
        if (symbolKey != null && symbolKey.length() > 0) {
            ...
            if (symbolKey.equals("0") && gcLayer.getType() != Layer.POINT) {
                ...
                // youhou, super heavy mechanism to put a size for each symbol
        ...
        umbmgr.generateUserMapfile((ColorGenerator)session.getAttribute(ObjectKeys.COLOR_GENERATOR));
        ...
        try {
            ...
            //LogEngine.log("GOClassRepresentation.process(): opening Mapserver connection with url: " + u);
