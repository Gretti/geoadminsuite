...
for UUCODE_FILE in `cat $1`; do
	...
	if [ ! -f $UUCODE*/trajetssimul.shp ] || [ ! -f $UUCODE*/trajetssimul.dbf ] || [ ! -f $UUCODE*/trajetssimul.shx ] ; then
		...
		if [[ $? -eq 1 ]] ; then
			...
			for PART in `echo "p1 p2 p3"` ; do
				# Fichier shapefile 2010 suivant plusieurs cas possible
				TRAJET_FILE_2010="$TMP_DIR_2010/Trajetssimul${UUCODE}${PART}"
				...
				if [[ $DB_STATUS_OK == "true" ]] ; then
					...
					if [[ $? == 0 ]] ; then
						...
						if [ $PRC_LONG = 't' ] ; then
						    ...
						    mv uu_${UUCODE}${PART}.zip $OUT_PATH/ok
						else 
						    echo "mauvaises stat, deplacement du resultat dans le rep KO :(("
						    mv uu_${UUCODE}${PART}.zip $OUT_PATH/ko
						fi
					
						if [[ $? == 0 ]] ; then
							echo "...................OK"
							echo "effacement du zip de resultat..."
							rm -f uu_${UUCODE}${PART}.zip
							testcmdstatus $?
							echo $CMD_STATUS
						else 
							echo "...................KO"
