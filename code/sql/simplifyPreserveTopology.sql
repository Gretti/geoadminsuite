...
returns setof record as $$
    ...
    BEGIN
        ...
        -- extract linestrings (rings) out of polygons. Simplification will be done on these linestrings.
        ...
        -- union the rings together to produce a clean network of linestring:
        -- union slipts linestrings at their intersection points
        create unlogged table gunion as select st_union(g) as g from rings;
        --create table gunion as select st_union(g) as g from rings;
        ...
        -- merge lines to remove duplicate edges that may exist after union       
        create unlogged table mergedrings as select st_linemerge(g) as g from gunion;
        --create  table mergedrings as select st_linemerge(g) as g from gunion;
        ...
        -- simplify the merged rings
        create unlogged table simplerings as select st_simplifyPreserveTopology(g, tol) as g from mergedrings;
        --create table simplerings as select st_simplifyPreserveTopology(g, tol) as g from mergedrings;
        ...
        -- spatial index before association
        ...
        -- error management
        BEGIN
            -- comparing percentage of overlaping area gives better results than PIP test.
            -- as input set is multipolygon, we first explode multipolygons into their polygons, to
            -- be able to find islands and set them the right source identifier.
        ...
        EXCEPTION 
            WHEN OTHERS THEN
                err := true;
                raise notice 'Error occured: %:%.', SQLSTATE, SQLERRM
                USING HINT = 'Function was not able to identify simplified polygons.'
                ||chr(10)||'The table simplepolys contains simplified polygons.'||chr(10)||'Try manual association between source and simplified polygons';
        END;
