...
create table trajet_2009_ok as (
	with tmp(tgid, idtraj, voirie_gid, link_id,  shape_length, voirie_geom, count) as (
		select distinct t.gid, t.idtraj, v.gid,  v.link_id, v.shape_length, v.geom, count(ordre) OVER (partition by t.idtraj)
		from trajet t, voirie_work v
		where t.code_gs::int = v.link_id
		and v.idtraj = t.idtraj
	)
	select t.idtraj, tmp.voirie_gid as gid, -1 as node_id, t.ordre, tmp.link_id, 
         tmp.shape_length as lgtron2009, t.lgtron2006, tmp.voirie_geom as geom
