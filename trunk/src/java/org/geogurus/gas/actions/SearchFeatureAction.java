        ...
        if (gc.getDatasourceType() != DataAccessType.ECW &&
            ...
            if (!gcEpsg.equals(mapEpsg)) {
                ...
                if (!mapEpsg.equals("900913")) {
                    ...
                    double x = Double.valueOf(lon).doubleValue();
                    double y = Double.valueOf(lat).doubleValue();
                    double tol = Double.valueOf(tolerance).doubleValue();
                    x = (x / originShift) * 180;
                    y = (y / originShift) * 180;
                    tol = (tol / originShift) * 180;
                    y = 180 / Math.PI * (2 * Math.atan(Math.exp(y * Math.PI / 180.0)) - Math.PI / 2.0);
                    tol = 180 / Math.PI * (2 * Math.atan(Math.exp(tol * Math.PI / 180.0)) - Math.PI / 2.0);
                    lon = String.valueOf(x);
                    lat = String.valueOf(y);
                    tolerance = String.valueOf(tol);
                }
            }
