 ...
 * ZipDownload.java
 *
 * Created on 21 octobre 2002, 14:45
 */
...
/**
 *  Servlet to export, zip and download geographic data
 * @author  gng + nri
 */
public class ZipDownload extends BaseServlet {
    ...
    public void process(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (exportType == EXPORT_TYPE_FULL) {
            ...
            files = new String[3];
            files[0] = getServletContext().getRealPath("") + "/msFiles/tmpMaps/user_" + session.getId() + ".map";
            files[1] = getServletContext().getRealPath("") + "/msFiles/templates/symbols.sym";
            files[2] = getServletContext().getRealPath("") + "/msFiles/fonts/font.list";
            ...
            File userMapfile = new File(getServletContext().getRealPath("") + "/msFiles/tmpMaps/user_" + session.getId() + ".map");
            ...
            try {
                ...
                debugToClient(response, "Error: Mapfile could not be prepared for download...");
            }
            return;
            ...
            files = new String[3];
            files[0] = gc.getDatasourceName() + fs + gc.getTableName();
    ...
    protected String[] wrapShp2Pgsql(GeometryClass gc, String dbName, String id, StringBuffer msg) {
        ...
        String sqlFile = getServletContext().getRealPath("") + fs + "msFiles" + fs + "tmpMaps" + fs + "tmp" + id + ".sql";

        // full path to the shapefile
        String dataFile = gc.getDatasourceName() + fs + gc.getTableName();
    ...
    protected String[] wrapPgsql2Shp(GeometryClass gc, String id, StringBuffer msg) {
        ...
        fileName += fs + "msFiles" + fs + "tmpMaps" + fs + gc.getTableName() + id;
    ...
    protected String[] generateSQLDump(GeometryClass gc, String dbName) {
        ...
        try {
            // sql out file:
            dataPath = getServletConfig().getServletContext().getRealPath("") + fs + "msFiles" + fs + "tmpMaps" + fs + gc.getTableName() + ".sql";
    ...
    public static void main(String[] args) {
        SysCommandExecutor cmdExecutor = new SysCommandExecutor();
        try {
            int exitStatus = cmdExecutor.runCommand("/chroots/cartoweb-2007-07-17/usr/bin/shp2pgsql -s 0 /home/gnguessan/projects/mediapost/working_docs/mediapost/data_dir/raster_bassinM5_et_M6/50k.shp50k fdsq");
            String cmdMessage = cmdExecutor.getCommandOutput();

            //PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(sqlFile)));
            //out.println(cmdMessage);
            System.out.println("output: " + cmdMessage + " - Error : " + exitStatus);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
