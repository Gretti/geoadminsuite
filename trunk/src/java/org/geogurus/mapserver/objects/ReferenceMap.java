...
public class ReferenceMap extends MapServerObject implements java.io.Serializable {
    ...
    public void setImage(File image_) {
        image = image_;
    }
    ...
    public void setMarker(String marker_) {
        marker = marker_;
    }

    public void setMarkerSize(int markerSize_) {
        markerSize = markerSize_;
    }

    public void setMinBoxSize(int minBoxSize_) {
        minBoxSize = minBoxSize_;
    }

    public void setMaxBoxSize(int maxBoxSize_) {
        maxBoxSize = maxBoxSize_;
    }
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (tokens[0].equalsIgnoreCase("COLOR")) {
                    ...
                    if (!result) {
                        MapServerObject.setErrorMessage("ReferenceMap.load: cannot load COLOR object");
                    }
                    ...
                    isExtent = true;
                    if (!result) {
                        MapServerObject.setErrorMessage("ReferenceMap.load: cannot load EXTENT object");
                    }
                } else if (tokens[0].equalsIgnoreCase("IMAGE")) {
                    if (tokens.length < 2) {
                        MapServerObject.setErrorMessage("ReferenceMap.load: Invalid syntax for IMAGE: " + line);
                        return false;
                    }
                    ...
                    if (!result) {
                        MapServerObject.setErrorMessage("ReferenceMap.load: cannot load OUTLINECOLOR object");
                    }
                } else if (tokens[0].equalsIgnoreCase("SIZE")) {
                    if (tokens.length < 3) {
                        MapServerObject.setErrorMessage("ReferenceMap.load: Invalid syntax for SIZE: " + line);
                        return false;
                    }
                    size = new Dimension();
                    ...
                    if (tokens.length < 2) {
                        MapServerObject.setErrorMessage("ReferenceMap.load: Invalid syntax for STATUS: " + line);
                        return false;
                    }
                    tokens[1] = ConversionUtilities.removeDoubleQuotes(tokens[1]);
                    if (tokens[1].equalsIgnoreCase("ON")) {
                        status = ReferenceMap.ON;
                    } else if (tokens[1].equalsIgnoreCase("OFF")) {
                        status = ReferenceMap.OFF;
                    } else {
                        MapServerObject.setErrorMessage("ReferenceMap.load: Invalid value for STATUS: " + line);
                        return false;
                    }
                } else if (tokens[0].equalsIgnoreCase("END")) {
                    return true;
                } else {
                    MapServerObject.setErrorMessage("ReferenceMap.load: unknown token: " + line);
                    return false;
                }
            ...
            logger.warning("ReferenceMap.load(). Exception: " + e.getMessage());
            e.printStackTrace();
            return false;
        }

        if (!isExtent) {
            MapServerObject.setErrorMessage("ReferenceMap.load: required EXTENT token missing or incomplete");
            return false;
        }
