...
public class Label extends MapServerObject  implements java.io.Serializable {
    ...
    /** Empty constructor */
    public  Label() {
        this(UNDEF, false, null, null, 0, null, null, 0, null, null, Label.SMALL, BITMAP, '\n');
    }
    
    
    
    /** Creates a new instance of Label */
    public Label(int angle_, boolean antialias_, RGB color_, String font_,
    int minDistance_, Dimension offset_, RGB outlineColor_, int position_,
    RGB shadowColor_, Dimension shadowSize_, int size_, int type_, char wrap_) {
        angle = angle_;
    ...
    // set methods
    public void setAngle(double angle_)                 {angle = angle_;}
    ...
    public void setBackgroundShadowColor(RGB backgroundShadowColor_) {backgroundShadowColor = backgroundShadowColor_;}
    public void setBackgroundShadowSize(Dimension backgroundShadowSize_) {backgroundShadowSize = backgroundShadowSize_;}
    ...
    public void setForce(boolean force_)               {force = force_;}
    public void setMaxSize(int maxSize_)               {maxSize = maxSize_;}
    public void setMinDistance(int minDistance_)        {minDistance = minDistance_ ;}
    public void setMinFeatureSize( int minFeatureSize_){minFeatureSize = minFeatureSize_;}
    public void setMinSize(int minSize_)               {minSize = minSize_;}
    public void setOffset(Dimension offset_)            {offset = offset_;}
    public void setOutlineColor(RGB outlineColor_)     {outlineColor = outlineColor_;}
    public void setPartials(boolean partials_)          {partials = partials_;}
    public void setPosition(int position_)              {position = position_;}
    public void setShadowColor(RGB shadowColor_)        {shadowColor = shadowColor_;}
    public void setShadowSize(Dimension shadowSize_)    {shadowSize = shadowSize_;}
    public void setSize(int size_)                      {size = size_;}
    public void setType(int type_)                  {type = type_;}
    public void setWrap(char wrap_)                    {wrap = wrap_;}
    
    // get methods
    public double getAngle()                { return angle ;}
    public boolean isAntialias()            { return antialias;}
    public RGB getBackgroundColor()         { return backgroundColor;}
    public RGB getBackgroundShadowColor()   { return backgroundShadowColor;}
    public Dimension getBackgroundShadowSize() { return backgroundShadowSize;}
    ...
    public boolean isPartials()             { return partials ;}
    public int getPosition()                { return position ;}
    public RGB getShadowColor()             { return shadowColor;}
    public Dimension getShadowSize()        { return shadowSize;}
    public int getSize()                    { return size ;}
    public int getType()                    { return type ;}
    public char getWrap()                   { return wrap;}
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (tokens[0].equalsIgnoreCase("ANGLE")) {
                    ...
                    else if (tokens[1].equalsIgnoreCase("CL")) position = this.CL;
                    else if (tokens[1].equalsIgnoreCase("CC")) position = this.CC;
                    else if (tokens[1].equalsIgnoreCase("CR")) position = this.CR;
                    ...
                    else if (tokens[1].equalsIgnoreCase("AUTO")) position = this.AUTO;
                    else return false;
                } 
                    ...
                    if (tokens.length<2) return false;
                    tokens[1] = ConversionUtilities.getValueFromMapfileLine(line);
                    if (tokens[1].equalsIgnoreCase("TINY")) size = this.TINY;
                    else if (tokens[1].equalsIgnoreCase("SMALL")) size = this.SMALL;
                    else if (tokens[1].equalsIgnoreCase("MEDIUM")) size = this.MEDIUM;
                    else if (tokens[1].equalsIgnoreCase("LARGE")) size = this.LARGE;
                    else if (tokens[1].equalsIgnoreCase("GIANT")) size = this.GIANT;
                    else size = Integer.parseInt(tokens[1]);
                } 
                else if (tokens[0].equalsIgnoreCase("TYPE")) {
                    if (tokens.length<2) return false;
                    tokens[1] = ConversionUtilities.getValueFromMapfileLine(line);
                    if (tokens[1].equalsIgnoreCase("BITMAP")) type = this.BITMAP;
            ...
            System.out.println("Label.load(). Exception: " +  e.getMessage());
            e.printStackTrace();
            return false;
        }
        return result;
    ...
    public synchronized boolean saveAsMapFile(java.io.BufferedWriter bw) {
        ...
        try {
            ...
            switch (position) {
                ...
                case CL:                bw.write("\t\t\t position CL\n"); break;
                case CC:                bw.write("\t\t\t position CC\n"); break;
                case CR:                bw.write("\t\t\t position CR\n"); break;
                case LL:                bw.write("\t\t\t position LL\n"); break;
                case LC:                bw.write("\t\t\t position LC\n"); break;
                case LR:                bw.write("\t\t\t position LR\n"); break;
                case AUTO:              bw.write("\t\t\t position AUTO\n"); break;
            }
            if (shadowColor!=null) {
            ...
            switch (size) {
                case TINY:              bw.write("\t\t\t size TINY\n"); break;
                case SMALL:             bw.write("\t\t\t size SMALL\n"); break;
                case MEDIUM:            bw.write("\t\t\t size MEDIUM\n"); break;
                case LARGE:             bw.write("\t\t\t size LARGE\n"); break;
                case GIANT:             bw.write("\t\t\t size GIANT\n"); break;
                default:                bw.write("\t\t\t size "+size+"\n");
            }
            switch (type) {
                case BITMAP:
