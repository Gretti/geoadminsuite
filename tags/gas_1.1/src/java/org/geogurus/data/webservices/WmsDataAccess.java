...
public class WmsDataAccess extends DataAccess {
    ...
    public WmsDataAccess(String host, String layer,
            Datasource owner) {
    ...
    public boolean loadMetadata() {
        ...
        // Stores metadata informations to avoid recurrent parsing later
        // on
        org.geotools.data.ows.Layer wmsLayer = getWmsLayer();
        if (wmsLayer.getBoundingBoxes().size() > 0) {
            Iterator<Object> iteCrs = wmsLayer.getBoundingBoxes().keySet()
                    .iterator();
            if (iteCrs.hasNext()) {
        ...
        ex = new Extent(crsEnv.getMinX(), crsEnv.getMinY(), crsEnv.getMaxX(),
                crsEnv.getMaxY());
        extent = ex;
    ...
    private org.geotools.data.ows.Layer getWmsLayer() {
        ...
        try {
            WebMapServer wms = ObjectCache.getInstance().getCachedObject(new NoOpCacheable(owner.getHost()), new GasWmsDatastoreFactory(logger), GasWmsDatastoreFactory
			.createParams(new URL(owner.getHost())));
            for (org.geotools.data.ows.Layer layer : wms.getCapabilities()
                    .getLayer().getChildren()) {
                if (layer.getName().equals(this.layerName))
                    return layer;
            }
        } catch (MalformedURLException e) {
        }
        throw new IllegalStateException("Unable to find WMS Layer: "
                + layerName);
    ...
    protected Layer createMSLayerInner(RGB color) {
        ...
        try {
            ...
            // WMS layer need some information in metadata that we can
            // either get from service
            // or from gc built while parsing service (@see
            // DatasourceManager.getWebServiceGeosources)
            if (wmsCap.getRequest().getGetMap().getFormats().size() > 0) {
                format = wmsCap.getRequest().getGetMap().getFormats().get(0)
                        .toString();
            }

            String strMetadata = "\"wms_server_version\"    \""
                    + wmsCap.getVersion() + "\"";
            strMetadata += "\n\t\t\"wms_format\"    \"" + format + "\"";
            strMetadata += "\n\t\t\"wms_srs\"    \"EPSG:" + SRID + "\"";
            strMetadata += "\n\t\t\"wms_name\"    \"" + name + "\"";
